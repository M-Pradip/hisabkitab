...
function normalizeItem(item) {
  ...
  const quantity =
    Number.isFinite(quantityValue) && quantityValue > 0
      ? Math.max(1, Math.round(quantityValue))
      : 1;
...
function buildSession({ id, createdAt, data = {} }) {
  ...
  return {
    ...
    taxAmount: Math.max(
      0,
      Math.round((Number(data.taxAmount) || 0) * 100) / 100,
    ),
    taxLabel: text(data.taxLabel),
    paymentProvider,
...
export async function createSession(initialData = {}) {
...
function removeParticipant(session, participantId) {
  const participants = session.participants.filter((participant) =>
    participant.id === participantId ? participant.role === "host" : true,
  );

  const claims = {};
  for (const item of session.items) {
    const nextIds = (session.claims?.[item.id] || []).filter(
      (id) => id !== participantId,
    );
...
function updateItem(session, itemId, item) {
  const nextItemId = text(itemId);
  const currentItem = session.items.find((entry) => entry.id === nextItemId);
  const nextItem = normalizeItem({ ...currentItem, ...item, id: nextItemId });
  ...
  return {
    ...session,
    items: session.items.map((currentItem) =>
      currentItem.id === nextItemId
        ? { ...currentItem, ...nextItem, id: nextItemId }
        : currentItem,
    ),
  };
}
...
function removeItem(session, itemId) {
  ...
  const items = session.items.filter((item) => item.id !== nextItemId);
  const claims = { ...session.claims };

  delete claims[nextItemId];
...
function setItems(session, items = []) {
...
function setReceiptTax(session, action = {}) {
  const amount = Number(action.taxAmount);

  return {
    ...session,
    taxAmount: Number.isFinite(amount)
      ? Math.max(0, Math.round(amount * 100) / 100)
      : 0,
    taxLabel: text(action.taxLabel),
  };
}
...
function toggleClaim(session, itemId, participantId) {
  ...
  const current = Array.isArray(session.claims?.[itemId])
    ? session.claims[itemId]
    : [];
...
export async function applySessionAction(id, action = {}) {
  ...
  switch (action.type) {
    ...
    case "add_item":
      ...
      break;
    case "update_item":
      nextSession = updateItem(nextSession, action.itemId, action.item);
      break;
    case "remove_item":
      nextSession = removeItem(nextSession, action.itemId);
      break;
    case "set_items":
      nextSession = setItems(nextSession, action.items);
      break;
    case "set_receipt_tax":
      nextSession = setReceiptTax(nextSession, action);
      break;
    case "set_payment_qr":
