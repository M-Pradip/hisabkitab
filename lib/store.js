...
function updateItem(session, itemId, updates) {
  if (!itemId) {
    return session;
  }
...
function deleteItem(session, itemId) {
  if (!itemId) {
    return session;
  }
...
export async function applySessionAction(id, action = {}) {
  ...
  switch (action.type) {
    ...
    case "add_item":
      ...
      break;
    case "update_item":
      nextSession = updateItem(nextSession, action.itemId, {
        quantity: action.quantity,
      });
      break;
    case "delete_item":
      nextSession = deleteItem(nextSession, action.itemId);
      break;
    case "set_payment_qr":
